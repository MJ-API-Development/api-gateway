        ...
        async def wrapper(*args, **kwargs):
            ...
            if api_key is not None:
                ...
                if api_keys_lookup(api_key, {}).get('requests_count') >= limit:
                    ...
                    raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail='Rate rate_limit exceeded')
...
async def is_resource_authorized(path_param: str, api_key: str) -> bool:
    ...
    with get_session()() as session:
        ...
        can_access_resource = await subscription.can_access_resource(resource_path=path_param, session=session)
